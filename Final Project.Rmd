...
dataset <-dataset1 %>%
  ...
  select(isvip, verified, ishighlighted, total_interaction, counts_kisses, counts_profilevisits)

dataset3<-data.frame(
  Category = c("isvip","ishighlighted","verified")
)



dataset%>%
  select(isvip, ishighlighted, verified, total_interaction)%>%
  filter(isvip==TRUE,ishighlighted==TRUE,verified==TRUE)%>%
  ggplot()+
  geom_boxplot(aes(x=dataset, y=total_interactions))+
  geom_boxplot() +
  labs(title = "How different app features affect Interaction",
       x = "Variables that may effect interaction",
       y = "Total Interaction") +
  theme_minimal()
...
ggplot(data, aes(x = Category, y = Value)) +
  geom_boxplot() +
  labs(title = "Boxplot with Three Categories",
       x = "Category",
       y = "Value") +
  theme_minimal()
